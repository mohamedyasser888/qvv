-- One immutable, server-selected starter per room. This is intentionally kept
-- on the existing rooms row so players and spectators reconnect to the same
-- match decision without a second match-code system.
ALTER TABLE rooms
  ADD COLUMN IF NOT EXISTS starting_team INTEGER CHECK (starting_team IN (1, 2)),
  ADD COLUMN IF NOT EXISTS starting_wheel_angle NUMERIC,
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM teams WHERE room_id = v_room.id AND ready = false) THEN
    -- Keep the coin toss server-side without depending on the optional
    -- pgcrypto extension (which is not enabled in every Supabase project).
    ...
    v_wheel_angle := COALESCE(
      ...
      4320 + CASE WHEN v_starting_team = 1 THEN 90 ELSE 270 END
    );
